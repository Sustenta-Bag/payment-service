...
describe('PaymentController', () => {
  ...
  describe('createPayment', () => {
    ...
    it('deve tratar erro interno', async () => {
      req.body = { userId: 'user1', items: [{ title: 'item', description: 'desc', quantity: 1, unitPrice: 10 }], payer: { name: 'payer' } };
      Payment.mockImplementation(() => { throw new Error('fail'); });
      await createPayment(req, res);
  ...
  describe('listPayments', () => {
    it('deve retornar lista de pagamentos', async () => {
      ...
      Payment.find = jest.fn().mockReturnValue({ sort: jest.fn().mockReturnThis(), skip: jest.fn().mockReturnThis(), limit: jest.fn().mockResolvedValue([{ _id: 'id1', toObject: () => ({}) }]) });
      hateoasUtils.generatePaymentLinks.mockReturnValue([]);
      hateoasUtils.createHateoasResponse.mockReturnValue({});
      require('../../src/utils/paginationUtils').getPaginationInfo = jest.fn().mockReturnValue({ _meta: {}, _links: {} });
  ...
  describe('cancelPayment', () => {
    ...
    it('deve cancelar pagamento com sucesso', async () => {
      req.params = { id: 'id1' };
      const payment = { _id: 'id1', orderId: 'order1', userId: 'user1', status: 'pending', save: jest.fn() };
  ...
  describe('refundPayment', () => {
    ...
    it('deve reembolsar pagamento com sucesso', async () => {
      req.params = { id: 'id1' };
      const payment = { _id: 'id1', orderId: 'order1', userId: 'user1', status: 'approved', save: jest.fn() };
      Payment.findById = jest.fn().mockResolvedValue(payment);
